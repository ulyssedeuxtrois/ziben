...
import { sendBoostExpiringSoon } from "@/lib/email";
...
export async function POST(request: NextRequest) {
  ...
  for (const event of events) {
    const email = event.organizer?.email ?? event.submitterEmail;
    if (!email || !event.boostedUntil) continue;

    try {
      await sendBoostExpiringSoon(email, {
        id: event.id,
        title: event.title,
